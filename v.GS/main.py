...
#---------------------------------------------------------------------------------------------------------#
        ...
        try:
            ...
            for idx, (camera_name, camera_info) in enumerate(self.camera_info_dict_temp.items()):
                if len(camera_info["IP"]):
                    ...
                    if len(cameras_grouped[thread_index]) >= 5:
                        thread_index += 1
    ...
    def live_camera_viewer_setup(self, reset = False):
        try:
            ...
            print(len(self.camera_info_dict_temp))

            viewer_grid = generate_camera_grid(len(self.camera_info_dict_temp))
            ...
            if self.connect_nvr_flag == True:
                for camera_name, camera_info in self.camera_info_dict_temp.items():
                    ...
                    if camera_name not in self.camera_view_list.keys():
                        ...
                        self.camera_view_list[camera_name] = Livepage_view(camera_name = camera_name, 
                                                                                    camera_num = str(camera_info["Num"]), 
                                                                                    stackedWidget = self.ui_main.stackedWidget,
                                                                                    camera_ip= camera_info["IP"])
    ...
    def Notification_status(self):
        try:
            ...
            if receive_data["data"] == False:
                self.create_fade_out_msg(msg="장시간 자리 비움으로 인해 재로그인이 필요합니다")
