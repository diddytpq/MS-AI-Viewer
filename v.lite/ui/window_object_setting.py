...
class Object_settint_Dialog(QDialog):
    def __init__(self, parent=None):
        ...
        cls_list = self.parent.camera_info_dict_temp["Cls"]
        conf_list = self.parent.camera_info_dict_temp["Conf"]
        # par_time_list = self.parent.camera_info_dict_temp["ParTime"]
    ...
    def close_window(self):
        self.parent.camera_info_dict_temp["Cls"][0] = self.object_setting_ui.person_active_bnt.isChecked()
        self.parent.camera_info_dict_temp["Cls"][1] = self.object_setting_ui.car_active_bnt.isChecked()
        self.parent.camera_info_dict_temp["Cls"][2] = self.object_setting_ui.fire_active_bnt.isChecked()
        self.parent.camera_info_dict_temp["Cls"][3] = self.object_setting_ui.par_active_bnt.isChecked()

        # self.parent.camera_info_dict_temp["ParTime"][0] = self.object_setting_ui.par_start_time.currentText()
        # self.parent.camera_info_dict_temp["ParTime"][1] = self.object_setting_ui.par_end_time.currentText()
        self.close()


def set_conf(intance, std, target, index):
    target.setValue(std.value())
    intance.parent.camera_info_dict_temp["Conf"][index] = std.value()
