...
from utils import Eng2kor, Kor2eng, save_info
...
class ScheduleDialog(QDialog):
    def __init__(self, parent=None):
        ...
        # if self._parent.live_page_worker_dict != None :
        #     for worker in self._parent.live_page_worker_dict.values():
        #         for camera_name in worker.caps.keys():
        #             self._parent.camera_img_temp[camera_name] = worker.caps[camera_name].get_frame()
        ...
        for camera_name, camera_info in self._parent.camera_info_dict_temp.items():
            ...
            if camera_name in self._parent.camera_info_dict_temp.keys():
                img = self._parent.camera_img_dict[camera_name]
                frame = cv2.resize(img, (self.schedule_page_camera_view_list[camera_name].width(), self.schedule_page_camera_view_list[camera_name].height()))
                self.schedule_page_camera_view_list[camera_name].set_img(frame, 0.5)

            else :
                self.schedule_page_camera_view_list[camera_name].setPixmap(QPixmap(u":/newPrefix/ui/images/ico_video_off.svg"))
                self.schedule_page_camera_view_list[camera_name].setAlignment(Qt.AlignCenter)

        self.schedule_ui.event_box.currentIndexChanged.connect(lambda index: self.change_time_table(index))

        self.schedule_ui.schedule_apply_bnt.clicked.connect(lambda click, instance = self._parent: self.apply_schedule(click, instance))
        self.schedule_ui.schedule_close_bnt.clicked.connect(lambda click, instance = self._parent: self.schedule_close_window(click, instance))
        self.schedule_ui.schedule_all_remove_bnt.clicked.connect(self.schedule_ui.schedule_time_table.clearSelection)

        
    def schedule_close_window(self, click, _parent):
        save_info(host=_parent.HOST, port=_parent.PORT, file_name="camera_info", info=_parent.camera_info_dict_temp)
        data = {"msg" : "ms_ai"}
        url = f'http://{_parent.HOST}:{_parent.PORT}/run_ms_ai'
    ...
    def keyPressEvent(self, event):
        if (event.key() == Qt.Key_S and event.modifiers() == Qt.ControlModifier) :
            self.apply_schedule(None, self._parent)
