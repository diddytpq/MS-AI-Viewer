...
class Video_Buffer:
    def __init__(self, pipe="video1", appsink_name="video_sink", resolution=(640,480), chg_fps_mode = False, fps = 30):
        ...
        if chg_fps_mode == True:
            ...
            # self.video_decode = f'! decodebin ! videorate ! capsfilter name=capsfilter0 caps=video/x-raw,framerate={fps}/1 ! videoconvert ! video/x-raw,format=(string)BGR ! appsink name={appsink_name} emit-signals=true sync=false max-buffers=1 drop=true'
...
class Connect_Camera_Group(QThread):
    ...
    def run(self) -> None:
        ...
        # self.caps = {camera_name: Video_Buffer(pipe=camera_info['pipe'], resolution=(1280,720), chg_fps_mode=True, fps = self.fps) for camera_name, camera_info in self.cameras.items()}
        # self.caps = {camera_name: Video_Buffer(pipe=camera_info['pipe'], resolution=(1920,1080), chg_fps_mode=True, fps = self.fps) for camera_name, camera_info in self.cameras.items()}
...
class Livepage_view(QLabel):
    ...
    def __init__(self, camera_num, camera_name, camera_ip, stackedWidget):
        super().__init__()
