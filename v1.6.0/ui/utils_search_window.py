...
class SearchDialog(QDialog):
    def __init__(self, parent=None, camera_name=None, alarm_time=None):
        ...
        save_info(host=self.parent.HOST, port=self.parent.PORT, file_name="camera_info", info=self.parent.camera_info_dict_temp)
    ...
    def get_alarm_info(self, click):
        ...
        for camera_name, camera_viewer in self.search_page_camera_view_list.items():
            if camera_viewer.checked == True:
                ...
                camera_num_list.append(camera_viewer.camera_num)
        ...
        data = {"msg":{
            "id" : self.parent.login_info_temp["NVR"]["ID"],
            "pw" : self.parent.login_info_temp["NVR"]["PW"],
            "ip" : self.parent.login_info_temp["NVR"]["IP"],
            "start_day" : start_day,
            "end_day" : end_day,
            ...
            "camera_num" : camera_num_list,
            "sort_type" : order,

            "detect_type" : detect_type_list,

        }}
        url = f'http://{self.parent.HOST}:{self.parent.PORT}/get-search-info'
        receive_data = requests.get(url, json=data).json()

        event_info_ori = receive_data["data"]
        ...
        for event_info in event_info_ori["events"]:
            if event_info['type'] == 70:
                ...
                text = QTableWidgetItem(str(alarm_type))
                text.setTextAlignment(Qt.AlignCenter)
                text.setFlags(Qt.ItemIsSelectable|Qt.ItemIsDragEnabled|Qt.ItemIsDropEnabled|Qt.ItemIsUserCheckable|Qt.ItemIsEnabled)
                self.search_ui.event_table.setItem(row_position, 2, text)

                text = QTableWidgetItem(str(detect_time))
                text.setTextAlignment(Qt.AlignCenter)
                text.setFlags(Qt.ItemIsSelectable|Qt.ItemIsDragEnabled|Qt.ItemIsDropEnabled|Qt.ItemIsUserCheckable|Qt.ItemIsEnabled)
    ...
    def search_viewer_playback(self, click):
        try:
            ...
            if selected_indexes:
                ...
                for _, camera_viewer in self.search_page_camera_view_list.items():
                    if camera_viewer.camera_name == camera_name:
                ...
                rtsp_url = f'{self.parent.login_info_temp["NVR"]["ID"]}:{self.parent.login_info_temp["NVR"]["PW"]}@{self.parent.login_info_temp["NVR"]["IP"]}/playback/video{camera_num}?start={video_time_start}&end={video_time_end}&speed={self.search_ui.time_video_time_speed_input.text()}'
                # print(rtsp_url)
