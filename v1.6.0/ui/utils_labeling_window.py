...
class Labeling_Viewer(QLabel):
    ...
    def paintEvent(self, event):
        ...
        if self.drawing_new_box and self.drag_start_pos and self.mouse_pos :
            ...
            if self.checked_cls_num is None:
                self.main_instance.create_fade_out_msg(std_window=self, msg="아무 버튼도 체크되지 않았습니다.\n클래스 번호를 지정하려면 하나의 버튼을 체크하세요.")
                return
    ...
    def create_new_box(self, end_pos):
        ...
        if self.checked_cls_num is None:
            self.parent.create_fade_out_msg(std_window=self, msg="아무 버튼도 체크되지 않았습니다.\n클래스 번호를 지정하려면 하나의 버튼을 체크하세요.")
            return
